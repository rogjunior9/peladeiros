...
export function MonthlyChargePanel() {
  ...
  const getStatusBadge = (user: UserStatus) => {
    ...
    if (payment.status === "PENDING") return <Badge className="bg-amber-500/15 text-amber-500 border border-amber-500/50">Pendente</Badge>;
        ...
        {list.map((user) => {
          ...
          return (
            <TableRow key={user.id} className="border-slate-800/30 hover:bg-slate-900/40 transition-colors">
              ...
              <TableCell className="text-slate-500 font-mono text-xs">{formatCPF(user.document)}</TableCell>
              <TableCell>
                <Select value={user.playerType || "CASUAL"} onValueChange={(v) => updateUserType(user.id, v)}>
                  <SelectTrigger className={`w-[130px] h-8 border-slate-800 text-xs ${user.playerType === "MONTHLY" ? "bg-amber-950/20 text-amber-500 border-amber-900/50" : user.playerType === "GOALKEEPER" ? "bg-indigo-950/20 text-indigo-400 border-indigo-900/50" : "bg-slate-900 text-slate-400"}`}>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-slate-900 border-slate-800 text-slate-200">
                    <SelectItem value="MONTHLY" className="text-amber-500">Mensalista</SelectItem>
  ...
  return (
    <Card className="bg-zinc-950 border border-white/5 ring-1 ring-accent/20">
      <CardHeader className="bg-accent/5 border-b border-white/5 pb-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex items-center gap-2 bg-slate-900/50 p-1.5 rounded-lg border border-slate-800/50">
            <div className="px-2 text-slate-500"><CalendarDays className="h-4 w-4" /></div>
      ...
      <CardContent className="p-0">
        ...
        <div className="bg-slate-900/50 p-4 border-t border-slate-900 flex justify-between items-center">
          ...
          <Button onClick={handleGenerateValues} disabled={generating || loading || pendingCount === 0} className="bg-amber-500 hover:bg-amber-600 text-slate-950 font-semibold">
      ...
      <Dialog open={cpfDialogOpen} onOpenChange={setCpfDialogOpen}>
        <DialogContent className="max-w-md bg-slate-950 border-slate-900 text-slate-100">
          ...
          <div className="space-y-2 py-2">
            ...
            <Input value={cpfValue} onChange={(e) => setCpfValue(e.target.value.replace(/\D/g, "").slice(0, 11))} placeholder="00000000000" className="bg-slate-900 border-slate-800 text-slate-100" />
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setCpfDialogOpen(false)} className="hover:bg-slate-800">Cancelar</Button>
